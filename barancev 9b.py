...
def test_example(driver):
    ...
    driver.find_element_by_css_selector("#box-apps-menu > li:nth-child(6)").click()

    mass12 = driver.find_elements_by_css_selector(
        "#content > div > div.panel-body > form > table > tbody > tr")
    yh = 0
    tg1 = mass12[yh].text.split(' ', 1)[1]
    tg = '' if ' ' not in tg1 else tg1.rsplit(' ', 1)[0]
    print(tg)
    #text_file = open("C:\\Users\\akolzin\\Desktop\\site\\site\\5.txt", "w")
    driver.find_element_by_link_text(tg).click()
    yh = yh + 1
    ...
    for x in mass:
        ...
        print(last)
        pp = x.text.split(' ', 1)[1]
        p = '' if ' ' not in pp else pp.rsplit(' ', 4)[0]
        print(p)
