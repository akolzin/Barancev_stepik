...
def test_example(driver):
    ...
    #time.sleep(3)
    driver.find_element_by_css_selector("#index_email").send_keys("89508286810")
    time.sleep(1)
    driver.find_element_by_css_selector("#index_pass").send_keys("loknroshorgrimazhoverlor1")
    time.sleep(1)
    ...
    mass1 = driver.find_elements_by_css_selector("#side_bar_inner > nav > ol > li")
    #print(mass1)
    print(len(mass1)) # Результат: 2
    print(mass1)
    time.sleep(1)
    mass1[1].click()
    print(mass1[1].text)
    #text_file = open("C:\\Users\\exe90\\Desktop\\3.txt", "w")
    for x in mass1:
        mass1[er].click()
        time.sleep(1)
        print(mass1[er].text)
        print(x.text)
        #x.find_element_by_id(x.id).click()
        #driver.find_element_by_link_text(x.text).click()
        href = driver.find_element_by_link_text(x.text).get_attribute("href")
        print(href) # Вывод: recomand.html
        time.sleep(1)
        print(x.id)
        # print(x.location)
        ...
        # text_file1 = open('C:\\Users\\exe90\\Desktop\\3.txt', 'a')
